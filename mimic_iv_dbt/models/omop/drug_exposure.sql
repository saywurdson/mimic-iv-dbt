...
-- ED meds (ed.medrecon + ed.pyxis) from lk_ed_drug_mapped; visit_occurrence_id via the ED visit
-- authority (lk_ed_visit, band 27) on stay_id; ids in the ed_drug band so existing ids are untouched.
SELECT
    {{ mimic_sk('ed_drug', 'src.subject_id, src.stay_id, src.start_datetime, src.source_code') }} AS drug_exposure_id,
    per.person_id                               AS person_id,
    src.target_concept_id                       AS drug_concept_id,
    CAST(src.start_datetime AS DATE)            AS drug_exposure_start_date,
    src.start_datetime                          AS drug_exposure_start_datetime,
...
FROM {{ ref('lk_ed_drug_mapped') }} src
INNER JOIN {{ ref('cdm_person_all') }} per ON CAST(src.subject_id AS VARCHAR) = per.person_source_value
LEFT JOIN {{ ref('lk_ed_visit') }} vis ON vis.stay_id = src.stay_id
WHERE src.target_domain_id = 'Drug'
